    ...
    <section class="portfolio-images">
        <div class="container">
            <% loop $PortfolioImages %>
                <div class="portfolio-images__item is-{$EvenOdd}<% if $FirstLast %> is-{$FirstLast}<% end_if %><% if $Content %> has-content<% end_if %><% if $ContentPosition = 'Left' %> is-left<% else_if $ContentPosition = 'Right' %> is-right<% else %> is-full<% end_if %>">
                    <div class="row">
