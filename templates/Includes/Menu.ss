<ul class="menu">
    <% loop $Menu(1) %>
        ...
        <% end_if %>
    <% end_loop %>
    <% if $SiteConfig.FormattedPhone %><li class="menu__item menu__item--icon"><a href="tel:{$SiteConfig.FormattedPhone}" class="menu__item__icon menu__item__icon--phone">{$SVG('phone-handset').extraClass('menu__item__icon__icon')}<span class="menu__item__icon__text">Phone</span></a></li><% end_if %>
    <li class="menu__item menu__item--icon"><button class="menu__item__icon menu__item__icon--menu toggle-menu">{$SVG('menu').extraClass('menu__item__icon__icon')}<span class="menu__item__icon__text">Menu</span></button></li>
</ul><!-- /.menu -->